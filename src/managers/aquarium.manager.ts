...
import {
  ICreateTankCommand,
  IUpdateTankCommand,
} from './interfaces/aquarium.manager.interface';
import { Tank } from '@entities/tank.entity';
import { Fish } from '@entities/fish.entity';
import { PaginatedResult } from '@core/types/pagination';

// Accessors
import type { ITankAccessor } from '@accessors/aquarium/interfaces/tank.accessor.interface';
import { TANK_ACCESSOR } from '@accessors/aquarium/interfaces/tank.accessor.interface';
...
import type { ISetupTankAccessor } from '@accessors/ai/interfaces/setup-tank.accessor.interface';
import { SETUP_TANK_ACCESSOR } from '@accessors/ai/interfaces/setup-tank.accessor.interface';
...
import { SuggestionTankIdeaEngine, TankSetupSuggestion } from '@engines/suggestion-tank-idea.engine';
import {
  CompatibilityEngine,
...
export class AquariumManager {
  constructor(
    ...
    @Inject(SETUP_TANK_ACCESSOR)
    private readonly setupTankAccessor: ISetupTankAccessor,

    // Engines - Pure Business Logic
    private readonly bioloadEngine: BioloadEngine,
    private readonly compatibilityEngine: CompatibilityEngine,
    private readonly suggestionTankIdeaEngine: SuggestionTankIdeaEngine
  ) { }

  async createTank(dto: ICreateTankCommand, userId: number): Promise<Tank> {
    const tank = new Tank();
    tank.fill({
      name: dto.name,
      length: dto.length,
  ...
  async updateTank(id: number, dto: IUpdateTankCommand): Promise<Tank> {
    const tank = await this.tankAccessor.findById(id);
    if (!tank) {
      throw new TankNotFoundException(id);
    }
  ...
  async getTankDashboard(tankId: number): Promise<TankDashboard> {
    ...
    const speciesInTank = [...new Set<string>(fishInTank.map((f) => f.species))]
  ...
  async setupTank(ideas: string): Promise<TankSetupSuggestion> {
    // 1. Engine: prepare system instruction, sanitize user input, get schema (pure logic)
    const systemInstruction = this.suggestionTankIdeaEngine.getSystemInstruction();
    const userPrompt = this.suggestionTankIdeaEngine.prepareUserPrompt(ideas);
    const jsonSchema = this.suggestionTankIdeaEngine.getResponseJsonSchema();

    // 2. Accessor: call external AI API (I/O only)
    const rawJson = await this.setupTankAccessor.generateContent(userPrompt, {
      jsonSchema,
      systemInstruction,
    });

    // 3. Engine: parse and validate response (pure logic)
    return this.suggestionTankIdeaEngine.parseTankSuggestion(rawJson);
  }
}
